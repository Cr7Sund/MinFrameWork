...
namespace UnityEditor.GraphView
{
    ...
    public struct SearchWindowContext
    {
        ...
        public SearchWindowContext(Vector2 screenMousePosition, VisualElement element, float requestedWidth = 0.0f, float requestedHeight = 0.0f)
        {
    ...
    public class SearchWindow : EditorWindow
    {
        ...
        private Rect GetTargetRect(SearchWindowContext context, float width, float height = 1)
        {
            return new Rect(context.screenMousePosition.x - width / 2, context.screenMousePosition.y - k_WindowYOffset, width, height);
        }
        ...
        void Init(SearchWindowContext context, ScriptableObject provider)
        {
            ...
            Rect buttonRect = GetTargetRect(context, width);
            CreateSearchTree();

            ShowAsDropDown(buttonRect, new Vector2(buttonRect.width, height));
            position = buttonRect;

            context.element.schedule.Execute(() =>
            {
                Rect rect = GetTargetRect(context, width, height);
                position = rect;
            });
